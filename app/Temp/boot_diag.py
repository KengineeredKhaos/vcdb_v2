...
def run_on_boot(app: Flask) -> None:
    ...
    Controlled by config:
      ...
      DEV_POLICY_HEALTH = True/False
      DEV_POLICY_HEALTH_LIST = True/False
    ...
    if bool(app.config.get("DEV_POLICY_HEALTH", True)):
        policy_health_summary(app)
...
def _print_policy_fingerprint(app: Flask) -> None:
    ...
    for ent in entries:
        ...
        pieces.append(f"{pkey}:{ver}:{eff}:{status}:{ph}:{sh}")
        rows.append(
            {
                "policy_key": pkey,
                "version": ver,
    ...
    if show_list and rows:
        ...
        for r in sorted(rows, key=lambda x: x["policy_key"]):
            ...
            if st == "deprecated":
                deprecated_in_catalog += 1

            print(
                f"  - {pk:24} v{ver:>2} eff={eff:10} "
                f"status={st:10} policy={ph} schema={sh}"
            )

        if deprecated_in_catalog:
            print(f"deprecated in catalog : {deprecated_in_catalog}")
    print("=====================================")


def policy_health_summary(app: Flask) -> None:
    """
    Dev-only summary of policy-health so split-policy breakage is visible at boot
    without having to run the CLI manually every time.
    """
    show_list = bool(app.config.get("DEV_POLICY_HEALTH_LIST", True))

    try:
        from app.extensions.policy_health import policy_health_report
    except Exception as exc:
        print("=== Governance policy health ===")
        print(f"unavailable         : {exc}")
        print("================================")
        return

    try:
        report = policy_health_report()
    except Exception as exc:
        print("=== Governance policy health ===")
    ...
    # Current policy_health_report() returns (warns, infos), not a dict.
    if isinstance(report, tuple) and len(report) == 2:
        ...
        print("=== Governance policy health ===")
        print(f"unexpected report   : {type(report).__name__}")
        print("================================")
        return
