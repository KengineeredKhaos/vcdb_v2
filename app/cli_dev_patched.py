...
# -----------------
# Policy Checks (v2 manifest-driven)
# -----------------

# Semantics/health checks for policies (optional layer).
# This remains useful for cross-policy invariants beyond JSON Schema.
try:
    from app.extensions.policy_semantics import (  # type: ignore
        PolicyError,
        policy_health_report,
    )
except (
...
def _resolve_governance_policy_base(
    app_root: Path, base: Path | None
) -> Path:
    """Resolve the Governance policy base folder.
    ...
    candidates = [
        ...
        app_root / "slices" / "governance",
        app_root / "slices" / "governance" / "policies",
    ]
    resolved = next((p for p in candidates if p.exists()), None)
    if not resolved:
        raise RuntimeError(
            "Could not find Governance policy base directory. Tried: "
            + ", ".join(str(p) for p in candidates)
        )
    return resolved


def _resolve_governance_schema_base(
    policy_base: Path, schema_base: Path | None
) -> Path:
    if schema_base and schema_base.exists():
        return schema_base

    candidates = [
        policy_base / "schemas",
        policy_base / "schema",  # tolerate legacy folder name
        policy_base,
    ]
    return next((p for p in candidates if p.exists()), policy_base)
...
def _load_governance_index(policy_base: Path) -> tuple[dict, Path]:
    """Load policy_governance_index.json from policy_base."""
    idx_path = policy_base / "policy_governance_index.json"
    if not idx_path.exists():
        raise RuntimeError(f"Missing governance index: {idx_path}")
    return _load_json_file(idx_path), idx_path


def _policy_paths_for_entry(
    policy_base: Path, schema_base: Path, entry: dict
) -> tuple[Path, Path]:
    policy_path = policy_base / entry["filename"]
    # schema_filename may include 'schemas/...'; interpret as relative to policy_base
...
def _format_schema_error(e) -> str:
    """Pretty one-line jsonschema error."""
    loc = "$"
...
def _validate_policies_v2(
    policy_base: Path, schema_base: Path
) -> tuple[list[str], list[str], list[str]]:
    """Validate all policies declared in governance_index against their schemas.

    Returns: (infos, warnings, fatals)
    """
    ...
    index, idx_path = _load_governance_index(policy_base)
    entries = list(index.get("policies") or [])
    if not entries:
        fatals.append("governance_index has no policies[] entries.")
        return infos, warns, fatals

    # Uniqueness + quick consistency checks
    ...
    for entry in entries:
        ...
        if not ppath.exists():
            ...
            continue
        if not spath.exists():
            fatals.append(f"{pkey}: missing schema file {spath}")
            continue

        try:
            payload = _load_json_file(ppath)
        except Exception as ex:
            fatals.append(f"{pkey}: invalid JSON in {ppath.name}: {ex}")
            continue
        ...
        if meta_key != pkey:
            fatals.append(
                f"{pkey}: meta.policy_key mismatch in {ppath.name} "
                f"(found {meta_key!r})"
            )
            continue
        ...
        if errs:
            fatals.append(f"{pkey}: schema validation failed ({ppath.name})")
            for e in errs[:25]:
                fatals.append(f"  - {_format_schema_error(e)}")
            if len(errs) > 25:
                fatals.append(f"  - ... and {len(errs) - 25} more")
            continue

        infos.append(f"{pkey}: OK ({ppath.name})")

    # One small friendliness: tell user where the index was loaded from
    infos.insert(0, f"Loaded governance_index from {idx_path}")

    return infos, warns, fatals


def _load_policy_by_key(policy_base: Path, key: str) -> dict:
    index, _ = _load_governance_index(policy_base)
...
def _issuance_rule_matches(rule: dict, sku_code: str, parts: dict) -> bool:
    """Match a v2 logistics_issuance sku_constraints rule against one SKU."""
    m = rule.get("match") or {}
    # classification_key match
    if "classification_key" in m:
        if classification_key_for(sku_code) != m["classification_key"]:
            return False
    # sku_parts match (policy uses semantic names)
    sku_match = m.get("sku_parts") or {}
    if sku_match:
        map_keys = {
            "category": "cat",
            "subcategory": "sub",
        ...
        for k, expected in sku_match.items():
            kk = map_keys.get(k, k)
            if parts.get(kk) != expected:
                return False
    return True


def _scan_logistics_issuance_coverage(policy_base: Path):
    """Coverage report for policy_logistics_issuance over InventoryItem catalog."""
    from types import SimpleNamespace as NS

    from app.extensions import db
    from app.slices.logistics.models import InventoryItem

    pol = _load_policy_by_key(policy_base, "logistics_issuance")
    ...
    for code in rows:
        if not code:
            continue
        if not validate_sku(code):
            bad_skus.append(code)
            continue

        try:
            parts = parse_sku(code)
        except Exception:
            bad_skus.append(code)
            continue
        ...
        any_hit = False
        for i, r in enumerate(rules):
            if _issuance_rule_matches(r, code, parts):
    ...
    summary = {
        ...
        "matched_any_rule": matched_any,
        "unmatched_any_rule": max(0, len(rows) - matched_any),
        "bad_skus": len(bad_skus),
        "bad_sku_samples": bad_skus[:10],
...
def _print_sku_policy_summary_v2(policy_base: Path) -> None:
    """Compare logistics_issuance allowed_units/allowed_sources vs DB usage."""
    ...
    allowed_units = set(sku_constraints.get("allowed_units") or [])
    allowed_sources = set(sku_constraints.get("allowed_sources") or [])

    click.echo("Logistics issuance — constraints vs catalog")
    click.echo("------------------------------------------")
    click.echo(
...
def _print_location_policy_summary_v2(policy_base: Path) -> None:
    """Compare policy_locations codes/patterns vs DB Location usage."""
    import re

    from app.extensions import db
    from app.slices.logistics.models import Location

    pol = _load_policy_by_key(policy_base, "locations")
...
@click.option(
    ...
    default=None,
    help="Override the policies base directory (folder that contains policy_*.json)",
)
@click.option(
    "--schema-base",
    type=click.Path(file_okay=False, dir_okay=True, path_type=Path),
...
def dev_policy_health(
    ...
    What this does:
      - Optional semantics checks via app.extensions.policy_semantics (if present)
      - JSON Schema validation for all policies in policy_governance_index.json
      - Logistics issuance coverage report over InventoryItem catalog
      - Location + unit/source summaries (DB vs policy)
    """
    echo_db_banner("policy-health")
    app_root = Path(current_app.root_path)

    # 1) Optional cross-policy semantics checks
    warns, infos = [], []
    try:
        w, i = policy_health_report()
        warns.extend(w or [])
        infos.extend(i or [])
    ...
    # 2) Schema validation (manifest-driven)
    pol_base = _resolve_governance_policy_base(app_root, base)
    sch_base = _resolve_governance_schema_base(pol_base, schema_base)
    v2_infos, v2_warns, fatals = _validate_policies_v2(pol_base, sch_base)
    infos.extend(v2_infos)
    warns.extend(v2_warns)

    # 3) Coverage + DB summaries (best-effort)
    ...
    if fatals:
        if as_json:
            ...
            for w in warns:
                click.echo(f"WARN: {w}")
            for f in fatals:
                click.echo(f"FATAL: {f}")
        raise SystemExit(1)

    if as_json:
        out = {"infos": infos, "warnings": warns}
        if coverage_summary is not None:
            out["logistics_issuance"] = {
                "summary": coverage_summary,
                "per_rule": coverage_per_rule,
            }
    ...
    click.echo("")
    _print_location_policy_summary_v2(pol_base)
...
@click.option(
    ...
    default="all",
    show_default=True,
    help="Policy key to lint (or 'all'). Uses policy_governance_index.json.",
)
@click.option(
    "--fix",
    is_flag=True,
...
def dev_policy_lint(
    ...
    pol_base = _resolve_governance_policy_base(app_root, base)
    sch_base = _resolve_governance_schema_base(pol_base, schema_base)
    index, _ = _load_governance_index(pol_base)
    ...
    for entry in entries:
        pkey = entry.get("policy_key") or "<?>"
        ppath, spath = _policy_paths_for_entry(pol_base, sch_base, entry)

        if print_paths:
            click.echo(f"{pkey:18s} json   : {ppath}")
        ...
        try:
            ...
            had_error = True
            click.secho(f"FAIL — {pkey} invalid: {ppath.name}", fg="red")
            click.echo(f"  {ex}")
