...
SKIP_DIRS = {".git", ".venv", "venv", "node_modules", "dist", "build", "migrations", "__pycache__", "tests_legacy"}
...
def has_kw(args_src, name):
    return re.search(rfr"\b{name}\s*=", args_src) is not None
...
def process_file(p: pathlib.Path):
    ...
    while True:
        ...
        if not span:
            out.append(src[m.end():]); break
        a0, a1 = span
        args_src = src[a0:a1]

        # only drop type= if both domain and operation are present
        if has_kw(args_src, "domain") and has_kw(args_src, "operation") and has_kw(args_src, "type"):
