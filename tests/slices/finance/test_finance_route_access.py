# tests/slices/finance/test_finance_route_access.py
...
def test_finance_routes_require_authentication(
    client,
    finance_seeded,
    path: str,
):
...
def test_finance_routes_deny_non_admin_users(
    client,
    finance_seeded,
    username: str,
    ...
    resp = client.get("/finance/hello", follow_redirects=False)
    assert_forbidden(resp)

    resp = client.get("/finance/journal", follow_redirects=False)
    assert_forbidden(resp)

    resp = client.get("/finance/activities", follow_redirects=False)
    assert_forbidden(resp)

    logout_if_possible(client)


def test_finance_routes_allow_admin(client, finance_seeded):
    login_and_settle_password(
        client,
